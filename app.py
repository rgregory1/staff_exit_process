...
# open form responses
sh = gc.open('Staff Exit Form (Responses)')
...
for staff in data:
    ...
    if staff['Sheet Setup'] == '':
        ...
        master_mat = master_list.get_all_values(returnas='matrix') # get all values from master worksheet
        occupied_master_rows = [x for x in master_mat if x[0] != ""] # remove empty rows from list
        ...
        staff_member_wks = master_sh.add_worksheet(staff['Staff Member'], src_worksheet=original_worksheet)
